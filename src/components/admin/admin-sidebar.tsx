...
export function AdminSidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col bg-space-deep border-r border-white/10 w-14 lg:w-56 flex-shrink-0">
      {/* Logo — lg+ only */}
      <div className="hidden lg:flex items-center px-4 h-14 border-b border-white/10 flex-shrink-0">
        <Logo variant="light" size="sm" />
      </div>
      {/* Spacer on md (icon-only mode) */}
      <div className="flex lg:hidden h-14 border-b border-white/10 flex-shrink-0" />
      ...
      <div className="border-t border-white/10 p-3">
        <div className="hidden lg:flex items-center gap-2 mb-3">
          <div className="w-8 h-8 rounded-full bg-blue-primary flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
        ...
        <button
          onClick={signOut}
          className="flex items-center gap-3 w-full px-3 py-2 rounded-lg text-text-light/60 hover:text-white hover:bg-white/5 transition"
          title="Sign out"
        >
